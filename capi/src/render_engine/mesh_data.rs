...
#[allow(non_snake_case)]
pub unsafe extern fn MeshData_SetVertexData(
	this: *mut GGD_MeshData,
	buffer: *const c_void,
	count: u32,
	format: GGVertexFormat,
) {
	...
	let future: Box<GpuFuture> = match format {
		...
		GGVertexFormat::UNDEFINED => {
			Box::new(this_ref.set_vertex_data(slice::from_raw_parts(buffer as *const u8, count as usize)).unwrap())
		},
	};
...
#[allow(non_snake_case)]
pub unsafe extern fn MeshData_SetIndexData(
	this: *mut GGD_MeshData,
	buffer: *const c_void,
	count: u32,
	format: GGIndexFormat,
) {
	let mut this_ref = Box::from_raw(this);

	let future: Box<GpuFuture> = match format {
		GGIndexFormat::SOUP_16U | GGIndexFormat::STRIP_16U => {
			Box::new(this_ref.set_vertex_data(slice::from_raw_parts(buffer as *const u16, count as usize)).unwrap())
		},
		GGIndexFormat::SOUP_32U | GGIndexFormat::STRIP_32U => {
			Box::new(this_ref.set_vertex_data(slice::from_raw_parts(buffer as *const u32, count as usize)).unwrap())
		},
		GGIndexFormat::UNDEFINED => {
			Box::new(this_ref.set_vertex_data(slice::from_raw_parts(buffer as *const u8, count as usize)).unwrap())
		},
	};
